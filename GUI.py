...
def openFile():
	global accelerates, from_amount, holds, length, main, multiplanets, planets, to_amount, twirls
	...
	if from_amount <= 1 or to_amount <= 1: # 1 planet LOL NO
		...
		for i, action in enumerate(main['actions']): # Enumerate all the actions
			if action['eventType'] == 'Hold': # Only record holds more than 1 lap
				holds[action['floor']] = {
...
def convertPlanets():
	...
	for i, action in enumerate(new_main['actions']): # Enumerate all the actions
		if action['eventType'] == 'SetSpeed' and planets[action['floor']] == -to_amount: # All planets to convert should be from_amount / to_amount speed
			new_main['actions'][i]['beatsPerMinute'] *= from_amount / to_amount
	...
	for child in interval_info.get_children(): # Enumerate all the children
		...
		if values[0] == 1: # I don't like to place a Block-1 SetSpeed, let's just change the whole level's BPM
			new_main['settings']['bpm'] *= from_amount / to_amount
			new_main['actions'].append({
				"eventType": "MultiPlanet",
				"floor": values[0],
				"planets": to_amount,
			...
			accelerate(values[0], from_amount, to_amount, True)
		if values[1] != length: # The goal needn't have any speed change
			accelerate(values[1], to_amount, from_amount)
			...
			if i > 0: # No need to convert Block 0
				...
				if planets[i] == -to_amount: # Convert minus amount
					...
					if holds.__contains__(i): # The degrees of the blocks before hold are not as same as after normal blocks
						hold_degree = degree + holds[i]['degree']
						if degree <= (from_amount - 2) * 180 / from_amount:
							hold_degree += 360
						hold_degree *= from_amount / to_amount # One lap used to be a 360° now is 240°
						degree = hold_degree % 360
						if degree <= (to_amount - 2) * 180 / to_amount:
							hold_degree -= 360
